...
import cxs03.z.connector.RequestStream;
...
public class HttpRequest implements HttpServletRequest {
    ...
    protected void parseParameters() {
        ...
        String contentType = getContentType(); //获取发送的内容类型
        if (contentType == null)
            contentType = "";
        ...
        //如果post了数据, 要检查是否在post中也有参数
        if ("POST".equals(getMethod()) && (getContentLength() > 0)
                && "application/x-www-form-urlencoded".equals(contentType)) {
            try {
                int max = getContentLength();
                int len = 0;
                byte buf[] = new byte[getContentLength()];//根据内容长度开辟定长缓冲区
                ...
                //对内容进行解析, results为引用, 解析结果直接保存到results中
